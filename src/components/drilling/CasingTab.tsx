...
import { CasingLithologySVG, CasingCutawaySVG } from './CasingSVGs';
...
export const CasingTab: React.FC<CasingTabProps> = ({
   ...
   const tensileSF = tensile / Math.max((currentCasing?.yieldStrength || 80000), 1);
   ...
   return (
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
         ...
         <div className="lg:col-span-8 space-y-6">
            ...
            {/* Live Animated Casing Lithology & Pore Pressure Profile */}
            <div className="glass-card rounded-3xl p-8 border-white/5 bg-black/40">
               <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Casing Lithology & Pore Pressure Profile</h4>
               <div className="h-[440px]">
                  <CasingLithologySVG
                     depth={casingDepth}
                     sfBurst={burstSF}
                     sfCollapse={collapseSF}
                     sfTensile={tensileSF}
                  />
               </div>
            </div>

            {/* Live Animated Casing Cutaway */}
            <div className="glass-card rounded-3xl p-8 border-white/5 bg-black/40">
               <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Casing Cross‑Section & Load Analysis</h4>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="h-[340px]">
                     <CasingCutawaySVG
                        grade={selectedGrade}
                        weight={selectedWeight}
                        internalP={internalPress}
                        externalP={externalPress}
                     />
                  </div>
                  <div className="space-y-3 text-[10px] font-mono text-slate-400 p-4 bg-white/5 rounded-2xl border border-white/5">
                     <p className="text-emerald-400 font-bold uppercase tracking-wider">Load Verification</p>
                     <p>Burst SF = P<sub>burst</sub> / &#916;P = {burstSF.toFixed(2)}</p>
                     <p>Collapse SF = P<sub>collapse</sub> / P<sub>ext</sub> = {collapseSF.toFixed(2)}</p>
                     <p className={burstSF >= 1.1 ? "text-emerald-400" : "text-rose-400"}>{burstSF >= 1.1 ? "✓ Burst OK" : "✗ Burst FAIL"}</p>
                     <p className={collapseSF >= 1.1 ? "text-emerald-400" : "text-rose-400"}>{collapseSF >= 1.1 ? "✓ Collapse OK" : "✗ Collapse FAIL"}</p>
                     <p className="text-[9px] text-slate-600 mt-2">API 5C3 / ISO 10400 Design Limits</p>
                  </div>
               </div>
            </div>
